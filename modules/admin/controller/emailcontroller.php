...
class EmailController extends Controller
{
    ...
    public function send(): void
    {
        ...
        $actions = ActionModel::fetchActiveWithLimit(0);
        ...
        if (RequestMethods::post('submitSendEmail')) {
            ...
            if (empty($errors) && $email->type == 1) {
                ...
                $users = UserModel::all(['active = ?' => true, 'deleted = ?' => false, 'role in ?' => $roles],
                    ['email']);
                $recipientsArr = [];
                ...
                $recipients = AttendanceModel::fetchUsersByActionId($actionId);
     ...
     * @param string $lang
     */
    public function loadTemplate($id, $lang = 'cs'): void
    {
        $this->disableView();
        ...
        $this->ajaxResponse($this->lang('COMMON_SUCCESS'), false, 200,
            ['text' => stripslashes($template->$fieldName), 'subject' => $template->getSubject()]);
    }
